...
- Household sync uses `POST /household-enrollments` only. Pending household records sync head-first; members retry with `HOUSEHOLD_HEAD_NOT_SYNCED` until the head acknowledgement propagates `householdId` to sibling records. Head **enrollment ID** equals client `householdCode`; members get `{householdCode}-{NN}` (e.g. `BAR-TAF-001-01`) with server-assigned `memberSequence`. Legacy standalone `POST /enrollments` create was removed.
