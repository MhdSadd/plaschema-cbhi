...
- Ward creation uses `POST /wards`; the form sends `name` and `lga`, while Plateau and Active remain fixed display values. When no ward exists in the LGA yet, the server derives a unique ward `code` as `<LGA_3>-<NAME_3>` (first three letters from each column, uppercased), suffixing `-2`, `-3`, … on collision. When a ward already exists in that LGA, the LGA prefix is taken from the first three letters of an existing ward code in that LGA instead of deriving it from the LGA name.
- Ward batch upload uses `POST /wards/batch` with a multipart `file` field. The UI accepts CSV, XLSX and XLS files up to 2 MB with columns `name` and `lga`, plus an optional `code` column; when `code` is present on a row it is used (normalized to uppercase), otherwise codes are derived server-side with the same LGA-prefix rule as single create, and the UI displays created, failed and row-level results.
