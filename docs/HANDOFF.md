...
- Ward creation uses `POST /wards`; the form sends `name`, `lga` (Plateau LGAs dropdown), and optional `status` (defaults to active on the server when omitted). Plateau remains a fixed display value. When no ward exists in the LGA yet, the server derives a unique ward `code` as `<LGA_3>-<NAME_3>` (first three letters from each column, uppercased), suffixing `-2`, `-3`, … on collision. When a ward already exists in that LGA, the LGA prefix is taken from the first three letters of an existing ward code in that LGA instead of deriving it from the LGA name.
- Ward batch upload uses `POST /wards/batch` with a multipart `file` field. The UI accepts CSV, XLSX and XLS files up to 2 MB with columns `name` and `lga`, plus an optional `code` column; when `code` is present on a row it is used (normalized to uppercase), otherwise codes are derived server-side with the same LGA-prefix rule as single create. Batch-created wards default to **inactive**. Re-uploading existing ward names (or matching explicit codes) is **idempotent**: rows are counted in `skipped`, not `failed`. The UI displays created, skipped, failed and row-level errors.
- Ward list data uses `GET /wards` with debounced server search, LGA and status filters, filter-scoped KPI cards (`meta.total`, `summary.active`, `summary.totalBeneficiaries`) and cursor-based Previous/Next navigation. Results are ordered by **LGA A–Z**, then ward name.
...
- Ward editing uses `PATCH /wards/:id`, field-worker assignment uses `PUT /wards/:id/field-workers`, and deletion uses `DELETE /wards/:id`. Changing a ward’s status also sets the same status on every health facility in that ward.
...
- Health-facility batch upload uses `POST /health-facilities/batch` with CSV/XLSX and XLS files up to 2 MB. Batch-created facilities default to **inactive**. Existing name+ward pairs are **idempotent** (`skipped`). Facility list API results are ordered by **LGA A–Z**, then facility name.
- Health-facility detail uses `GET /health-facilities/:id/detail` for overview statistics, capitation history and activity.
- Facility editing/status changes use `PATCH /health-facilities/:id`, and deletion uses `DELETE /health-facilities/:id` with confirmation. Changing a facility’s status also sets the same status on its ward and every other health facility in that ward. The admin UI warns before linked status changes from ward/facility edit dialogs and the facility activate/deactivate action.
...
- Field-worker creation uses `POST /users` with the role fixed to `field_worker`, optional multi-ward assignment (within one LGA) and active/inactive status. Phone is required and must be exactly 11 digits (local format, e.g. 080…). New field workers are created with `isPasswordChangeRequired: true`.
- Empty field-worker ward assignment intentionally means access to all wards and is labelled explicitly throughout the UI.
- Field-worker create/edit pick an LGA first, then wards in that LGA only; changing LGA clears ward selections. The API rejects assigned wards that span more than one LGA.
...
- Capitation uses tier-based flat monthly amounts per facility (not per-enrollee multiplication). Default bands: 1–5,000 → N650,000; 5,001–10,000 → N830,000; 10,001+ → N1,000,000. Facilities with zero active enrollees receive N0.
...
- Admin enrollment and ID-card category filters use the same fixed programme categories as the PWA: Elderly 65 and above, Indigent (very poor) / others, IDP, Orphan, People Living with Disability, Pregnant Woman, and Under 5 YEAR. Legacy labels (Internal displaced persons, IDPs, Elderly 65+, Indigents / Very Poor / Others) are normalized on ingest and in dashboard breakdowns.
- Files replaces Settings in the sidebar and uses `/file-jobs`, `/file-jobs/:id`, `/file-jobs/:id/download` and `DELETE /file-jobs/:id` for progress, fresh download links and permanent deletion. Deleting a completed or failed job removes its generated PDF/XLSX from object storage as well as the database row; ID-card jobs leave enrollees marked as printed. The Settings route remains available but unlisted.
...
- Household sync uses `POST /household-enrollments` only. Pending household records sync head-first; members retry with `HOUSEHOLD_HEAD_NOT_SYNCED` until the head acknowledgement propagates `householdId` to sibling records. Head **enrollment ID** is `PL-CBHI-{householdCode}`; members get `{headEnrollmentId}-{NN}` (e.g. `PL-CBHI-BSA-BAK-0001-01`) with server-assigned `memberSequence`. Legacy standalone `POST /enrollments` create was removed.
...
- `POST /api/household-enrollments` creates a household head or member (idempotent via `idempotencyId`). Head requests include `household.householdLocalId`, `household.householdCode`, `household.role=head`, and optional `sharedResidentialAddress`; the head’s public enrollment ID is `PL-CBHI-{householdCode}`. Member requests include `household.role=member` plus `householdId` when known. Returns the slim acknowledgement plus `householdId`, `householdRole`, `memberSequence`, and `householdCode`. `409 HOUSEHOLD_HEAD_NOT_SYNCED` when a member arrives before the head is persisted; `409 ENROLLMENT_ID_TAKEN` when the head ID already exists.
