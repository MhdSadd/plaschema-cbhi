...
export function CreateFieldWorkerDialog({ open, onCreated, onOpenChange }: CreateFieldWorkerDialogProps) {
  ...
  const { control, register, handleSubmit, reset, setValue, formState: { errors } } = useForm<Values>({ resolver: zodResolver(schema), defaultValues })
  const passwordMode = useWatch({ control, name: 'passwordMode' })
  const selectedWardIds = useWatch({ control, name: 'assignedWardIds' })

  const wards = useMemo(() => {
    const items = wardsQuery.data?.pages.flatMap((page) => page.items) ?? []
    return lga ? items.filter((ward) => ward.lga === lga) : items
  }, [lga, wardsQuery.data])
    ...
    <Dialog.Root onOpenChange={changeOpen} open={open}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 max-h-[92vh] w-[calc(100%-2rem)] max-w-2xl -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-2xl bg-card shadow-2xl outline-none" onEscapeKeyDown={(event) => mutation.isPending && event.preventDefault()} onInteractOutside={(event) => mutation.isPending && event.preventDefault()}>
          <div className="flex items-start justify-between border-b border-border px-6 py-5"><div><Dialog.Title className="text-lg font-semibold">Add Enrollment Officer</Dialog.Title><Dialog.Description className="mt-1 text-sm text-muted-foreground">Create an account and choose its ward access.</Dialog.Description></div><Button aria-label="Close create enrollment officer dialog" disabled={mutation.isPending} onClick={() => changeOpen(false)} size="icon" variant="ghost"><X aria-hidden="true" /></Button></div>
          <form onSubmit={handleSubmit(submit)}>
            <div className="grid gap-5 px-6 py-5">
              {mutation.isError && <div className="rounded-lg border border-destructive/30 bg-destructive/10 px-3 py-2 text-sm text-destructive" role="alert">{getApiErrorMessage(mutation.error, 'Unable to create the enrollment officer.')}</div>}
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div><label className="mb-1.5 block text-sm font-semibold" htmlFor="create-worker-phone">Phone Number</label><Input {...register('phone', { onChange: () => mutation.reset() })} aria-invalid={Boolean(errors.phone)} id="create-worker-phone" inputMode="numeric" maxLength={11} placeholder="e.g. 08034567890" />{errors.phone && <p className="mt-1 text-xs text-destructive">{errors.phone.message}</p>}</div>
              ...
              <FieldWorkerWardAccessFields
                ...
                emptySelectionNote="If none are selected, this worker can access all wards."
                lga={lga}
              ...
              {selectedWardIds.length === 0 && (
                <div className="rounded-lg border border-amber-300 bg-amber-50 px-3 py-2 text-xs text-amber-900">
                  No wards selected: this account will have access to all wards.
                </div>
              )}
            ...
            <div className="flex gap-3 px-6 pb-6"><Button className={`${btnSecondary} flex-1`} disabled={mutation.isPending} onClick={() => changeOpen(false)} type="button" variant="outline">Cancel</Button><Button className={`${btnPrimary} flex-1`} disabled={mutation.isPending || wardsQuery.isError} type="submit">{mutation.isPending ? <><LoaderCircle aria-hidden="true" className="animate-spin" /> Creating…</> : 'Create Enrollment Officer'}</Button></div>
          </form>
        </Dialog.Content>
      </Dialog.Portal>
    </Dialog.Root>
