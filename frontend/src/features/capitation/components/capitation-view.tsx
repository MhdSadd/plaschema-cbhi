  ...
  return (
    <div className="flex flex-1 flex-col gap-6 p-4 sm:p-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold tracking-[-0.48px]">Capitation</h1>
          <p className="mt-0.5 text-sm text-muted-foreground">
            Calculate and review monthly capitation for active healthcare facilities.
          </p>
        </div>
        <Button className={btnPrimary} onClick={() => setShowGenerate(true)}>
          <Plus aria-hidden="true" /> {hasRun ? 'Regenerate capitation' : 'Generate capitation'}
        </Button>
      </div>

      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {[
          ['Facilities', totals.totalFacilities.toLocaleString()],
          ['Beneficiaries', totals.totalBeneficiaries.toLocaleString()],
          ['Capitation', formatNaira(totals.totalCapitation)],
        ].map(([label, value]) => (
          <div className={`rounded-xl bg-card p-5 ${cardShadow}`} key={label}>
        ...
        <div className={`rounded-xl bg-card p-5 ${cardShadow}`}>
          <p className="text-xs font-medium text-muted-foreground">Capitation bands</p>
          {query.isPending ? (
            <Skeleton className="mt-2 h-16 w-full" />
          ) : (
            <div className="mt-2">
              <CapitationBandsSummary compact tiers={summary?.tiers} />
            </div>
          )}
        </div>
      </div>
      ...
      {summary && (
        <p className="-mt-3 text-xs text-muted-foreground">
          Latest run generated {formatLagosDate(summary.generatedAt)}
        </p>
      )}
      {query.isError && query.data && (
        <p
          ...
          role="alert"
        >
          The latest capitation update failed. The previous results remain visible.{' '}
          <button className="font-semibold underline" onClick={() => void query.refetch()} type="button">
            Retry
          </button>
        </p>
      )}
      ...
      <div className="flex flex-wrap items-end gap-3">
        <label className="grid gap-1 text-xs font-medium text-muted-foreground">
          ...
          <select
            ...
            value={month}
          >
            {CAPITATION_MONTHS.map((label, index) => (
        ...
        <div className={searchBar} style={{ flex: '1 1 240px', maxWidth: '300px' }}>
          <Search aria-hidden="true" className="size-4" />
          <input
            aria-label="Search capitation facilities"
        ...
        <select
          aria-label="Filter capitation by LGA"
        ...
        {query.isFetching && (
          <LoaderCircle
            aria-label="Updating capitation"
        ...
        <span className="ml-auto text-sm text-muted-foreground">
          {query.isPending
            ? 'Loading records…'
            : `Showing ${records.length} of ${totalCount.toLocaleString()} facilities`}
        </span>
      </div>
      ...
      <div className={`overflow-hidden rounded-xl bg-card ${cardShadow}`}>
        {query.isError && !query.data ? (
          <div
            ...
            role="alert"
          >
            <p className="font-semibold">Unable to load capitation.</p>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr>
                  {['Facility', 'LGA', 'Period', 'Beneficiaries', 'Band', 'Amount', 'Action'].map(
                    (heading) => (
                ...
                {query.isPending
                  ...
                  : records.map((record) => (
                      <tr className="hover:bg-muted/40" key={record.id}>
                        <td className={`${tdCell} font-semibold`}>{record.facilityName}</td>
                        <td className={`${tdCell} text-muted-foreground`}>{record.lga}</td>
                        <td className={`${tdCell} text-muted-foreground`}>{record.period}</td>
                        <td className={tdCell}>{record.beneficiaryCount.toLocaleString()}</td>
                        <td className={tdCell}>{formatRecordBand(record)}</td>
                        <td className={`${tdCell} font-semibold`}>{formatNaira(record.amount)}</td>
                        <td className={tdCell}>
                          <Button
                            onClick={() => navigate(`/admin/facilities/${record.healthFacilityId}`)}
                            size="sm"
                            variant="ghost"
                          >
                            View facility
                    ...
                    <td className="px-6 py-16 text-center" colSpan={7}>
                      <p className="font-semibold">
                        {debouncedSearch || lga
                          ...
                          : `No capitation run is available for ${periodLabel}.`}
                      </p>
                      <p className="mt-1 text-sm text-muted-foreground">
                        {debouncedSearch || lga
                          ? 'Change the search or LGA filter and try again.'
                          : 'Generate a run to calculate capitation for active facilities.'}
                      </p>
                      {!debouncedSearch && !lga && (
                        <Button className={`${btnPrimary} mt-4`} onClick={() => setShowGenerate(true)}>
                          <Plus aria-hidden="true" /> Generate capitation
                        </Button>
                      )}
                    </td>
                  </tr>
        ...
        {!query.isError && (
          <div className="flex items-center justify-between border-t border-border px-4 py-3">
            ...
            <div className="flex gap-2">
              ...
              <Button
                disabled={!meta?.hasMore || !meta.nextCursor || query.isFetching}
                onClick={nextPage}
                variant="outline"
              >
